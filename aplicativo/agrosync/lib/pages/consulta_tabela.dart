...
class _ConsultaTabelaState extends State<ConsultaTabela> {
    ...
    setState(() {
      _filteredPlants = _plants.where((plant) {
        final matchesDate = _filterDateController.text.isEmpty ||
            (plant['Data']
                    ?.toLowerCase()
                    .contains(_filterDateController.text.toLowerCase()) ??
                false);
        final matchesPasture = _filterPastureController.text.isEmpty ||
            (plant['Pasto']
                    ?.toLowerCase()
                    .contains(_filterPastureController.text.toLowerCase()) ??
                false);
        final matchesSpecies = _filterSpeciesController.text.isEmpty ||
            (plant['Espécie']
                    ?.toLowerCase()
                    .contains(_filterSpeciesController.text.toLowerCase()) ??
                false);
        final matchesCondicao = _filterCondicaoSoloController.text.isEmpty ||
            (plant['Condição do Solo']?.toLowerCase().contains(
                    _filterCondicaoSoloController.text.toLowerCase()) ??
                false);
        final matchesCulture = _filterCultureController.text.isEmpty ||
            (plant['Cultura']
                    ?.toLowerCase()
                    .contains(_filterCultureController.text.toLowerCase()) ??
                false);
  ...
  void _refreshItems() {
    final data = _plantBox.keys.map((key) {
      ...
      return {
        ...
        "Condição do Solo":
            item["Condição do Solo"] ?? item["Condição da Área"],
              ...
              onPressed: () async {
                if (_formKey.currentState!.validate()) {
                  final updatedPlant = {
                    ...
                    "Peso Verde":
                        double.tryParse(freshWeightController.text) ?? 0.0,
                    "Peso Seco":
                        double.tryParse(dryWeightController.text) ?? 0.0,
                  };
                  ...
                  child: Scrollbar(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final plant = _filteredPlants[index];
                        final species =
                            plant['Espécie'] ?? "Espécie não disponível";
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  ...
                                  try {
                                    ...
                                    _showSnackbar(
                                        "Erro ao deletar do Firestore: $e");
                                  }
  ...
  Widget _buildTextField({
    ...
    List<TextInputFormatter>?
        inputFormatters, // <-- Adicione este parâmetro opcional
  }) {
    return Padding(
  ...
  Map<String, dynamic> firestoreToHive(
  ...
  String? getDropdownValue(
      TextEditingController controller, List<DropdownMenuItem<String>> items) {
